...
public class TestRun1 {
	public static void main(String[] args) {
		...
		// 부모 참조 변수 = 부모 객체
		// 양쪽 다 Parent 타입 == 컴퓨터 값 처리 원칙이 위배되지 않고 잘 지켜진다
...
//		System.out.println(p2.getCar()); // Child 메서드 접근 불가
		...
		/* 다운 캐스팅 (Down Casting)
		 * - 업 캐스팅 상태에서 (부모 참조 = 자식 객체)참조 변수의 자료형을 자식 객체 자료형으로 강제 형번환 하는 것
		 */
		...
		// 아래 방법 처럼 필요할 때 마다 형 변환 가능 하지만 가독성이 떨어져 보통 자식 참조 변수에 대입해 사용
		// ((Child)p3).getCar();
		System.out.println(c3.getCar()); // 자식 메서드 참고 가능
		 ...
		 * 
		 * - 참조 하고 있는 객체가 맞는지 확인(true / false)
		 * 
		 * *주의 사항*
		 ...
		 * 
		 * p instanceof Child -> p가 참조하는 객체가 Child야? true
		 * 
		 * p instanceof Parent -> p가 참조하는 객체가 Parent야? true
		 * 
		 * ==> 주의 사항을 잘 이용하면 Parent, Child 클래스가 상속 관계가 맞는지 확인하는 용도로 사용 가능
		 */
		// 향상된 for문
		for(Object obj : arr) {
			
			/* tip : 자료형을 연달아 검사하는 경우 마지막 자식부터 검사하는게 좋다 */
			// 얻어온 요소가 Child 타입이면 
