...
public class VariableEx3 {
	public static void main(String[] args) {
		...
		int num1 = 290;
		byte result1 = (byte)num1; // byte 단위로 강제 형변환
		...
		// 실수를 정수로 강제 형변환 -> 소수점 버림 -> 실수를 정수로 변환-> 정수 부분만 확인(손실 활용)
		double num2 = 123.456789;
		int result2 = (int)num2; // int로 강제 형변환
		
		System.out.println("num2 : " + num2); // 123.456789
		System.out.println("result2 : " + result2); // 123
		// 자동 형변환
		System.out.println(num2 - result2); // 0.456789 -> 0.45678900000000056 (2진수 소수 표현 시 문제점)
		...
		// 1~10 사이 난수(정수) -> 강제 형변환을 이용해 소수점 제거
		...
		System.out.println("이의 번호 : " + (int)'이'); // 51060
		System.out.println("소의 번호 : " + (int)'소'); // 49548
		System.out.println("진의 번호 : " + (int)'진'); // 51652
		System.out.println("ㄱ의 번호 : " + (int)'ㄱ'); // 12593
		System.out.println("ㄱ + 1 : " + (char)('ㄱ' + 1)); // ㄲ
		System.out.println("ㄱ + 2 : " + (char)('ㄱ' + 2)); // ㄳ
		
		System.out.println("-----------------");
		System.out.println("[오버 플로우]");
		// 오버 플로우(Overflow) : 연산 결과가 자료형의 값의 범위를 초과(+)한 경우
		// 언더 플로우(Underflow) : 오버플로우의 반대 경우(-)
