 ...
 * 상속 키워드 : extends(확장하다) -> 부모 코드를 물려 받은 자식의 크기가 커지기 때문
 * 
 * *주의 사항*
 * - 부모의 코드 중 생성자는 물려받지 못함
 * - 부모의 private 접근 제한자 필드 / 메서드는 상속 받긴 하지만 직접 접근 불가
 * -> 상속 관계여도 다른 클래스(객체)로 인식되기 때문에 접근 불가 
 */
...
public class Child1 extends Parent{
	 ...
	 * - super()는 자식 생성자에만 작성할 수 있으며, 반드시 첫 번째 줄에 작성해야 한다
	 */
	...
	public void print() {
		...
		// protected 필드는 상복 받은 클래스에서 직접 접근 가능
	...
	/* super 참조변수
	 * - 자식 객체 내부의 부모 갹채를 참조하는 변수
	 * 
	 * - 사용 시기 : 부모의 필드 / 메서드임을 확실하게 명시할 때 주로 사용
	 */
	 ...
	 * Riding : 터는 것
	 * 
	 * - 부모로 부터 물려 받은 메서드를 자식이 같은 이름의 메서드를 만들어서 덮어쓰기하는 것
	 * -> 물려 받은 자식에 맞게 부모의 메서드를 다시 정의해야되는 경우 사용
	 * 
	 * *중요*
	 * @Override 어노테이션(Annotation)
	 * 1) 해당 메서드는 오버라이딩 되었음을 명시
	 * 2) 오버라이딩된 코드가 알맞게 작성되었는지 컴파일러에게 검사 하라고 지시하는 컴파일러 주석 
	 * 
	 * [오버라이딩 성립 조건]
	 * 1. 접근 제한자는 같거나 더 넓은 범위만 가능
	 * 2. 반환형, 매개 변수, 메서드명은 모두 동일해야한다
	 * 3. 예외처리 구문(throws)은 같거나 좁은 범위
	 * 
	 * (쉽게 말해 부모랑 똑같이 쓰는게 제일 쉬움)
	 */
	...
	public String toString() {
		...
		// Child1 클래스 내부에서 toString() 호출하면 Child1.toString()이 호출됨
		// super.toString() : 부모 클래스에 작성된 toString() 호출
		// -> 오버라이딩 했다고 해서 부모의 메서드가 덮어쓰기 되어 사라지는 것이 아님
