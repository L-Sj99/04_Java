 ...
 * 
 * - 클래스 선언부에 아무런 상속 구문(extends)이 작성되어 있지 않으면 컴파일러가 extends Object 구문을 추가해준다 
 */
...
public class Parent extends Object{
	...
	/* protected 접근 제한자 : 같은 패키지 또는 상속 관계 자식 클래스
	 * - 같은 패키지 : 객체 생성 후 사용
	 * ex) 자식 클래스에서 Parent p = new Parent(); p.lastName;
	 * 
	 ...
	 * - Object.toString() 역할
	 * -> 물려받은 자식이 오버라이딩 하여 객체가 가지고 있는 필드 값을 하나의 문자열로 만드는 것(약속 되어있음)
	 * 
	 * -> print() 관련 구문에서 ()안에 참조 변수명만 작성해도 자동으로 toString()이 호출됨
	 */
	@Override // 오버라이딩 잘 되었는지 컴파일러가 알아서 검사
	public String toString() {
		return String.format("[Parent] %s / %s / %d / %s", lastName, address, money, car);
	}
	
	/* final 메서드
	 * - 오버라이딩 불가 -> 부모의 메서드가 완벽해서 또는 재정의하면 문제가 되기에 더 이상 오버라이딩을 못하게 함(마지막으로 정의된 메서드)
	 * 
