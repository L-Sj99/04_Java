...
public class TryCatchService {
	...
	/* Exception(예외) : 코드로 해결 가능 한 에러 (if, try-catch, throws 등)
	 * Exception Handling(예외 처리) : 예외 발생 시 이를 처리하는 구문(try-catch, throws)
	 */
	/* 고의적으로 예외를 발생 시키는 코드 작성 */
	public void test1() {
		// NullPointerException -> 참조하고 있는 객체가 없는데 객체의 필드 / 메서드를 호출할 때 던져지는 예외
		// String str = null;
		// str.split("/");
	 ...
	 * <pre>
	 * - try {} : 예외가 발생할 가능성이 있는 코드를 {}내부에 작성 시도
	 * - catch(예외 참조 변수) {} : try에서 던져진 예외를 ()에 작성된 참조변수로 잡아서 처리하는 구문
	 * </pre>
	 */
...
public void test2() {
	...
	try {
		...
		// try에서 던져지게 될 InputMismatchException 객체를 잡아서 처리하는 구문
			...
			// 발생된 예외를 처리했기 때문에 어떤 이유로 어디서 무슨 예외가 발생 했는지 콘솔에 빨갛게 뜨는 stackTrace 구문에 출력되지 않음
	...
	public void test4() {
		try {
			...
			System.out.println("몫 : " + (num1 / num2));
		} catch(InputMismatchException e) { // 자식 타입 예외를 먼저 처리(if) -> 부모타입을 나머지 예외를 처리하는(else) 형식으로 사용
			System.out.println("int 범위 정수만 입력해줘");
		} catch(RuntimeException e) {
			// catch에 매개 변수 타입을 부모 타입으로 설정 -> 던져지는 모든 자식 타입의 예외를 잡아서 처리
			// -> 예외 처리에 다형성(업 캐스팅) 적용
			// 장점 : 여러 예외처리 구문을 작성하지 않고도 예외처리가 가능
			// 단점 : 발생하는 예외별 처리 방법을 제시할 수 없다
			System.out.println("뭔지 모르겠지만 예외 발생");
			
			// InputMismatchException에 빨간줄(컴파일 에러이 발생하는 이유
			// : 앞에서 부모 타입 참조변수를 이용해 자식 타입 예외를 잡아서 처리했기 때문에 도달할 수 없는 코드(dead code)가 되어버림
	 ...
	 * 1) Checked Exception : 반드시 예외 처리 구문을 작성해야하는 예외
	 * - 예외처리 구문 미작성 시 컴파일 에러 발생(빨간줄)
	 * 
	 * 2) Unchecked Exception : 예외 처리 구문 작성 여부가 필수는 아닌 선택하는 예외
	 * - 예외 처리 구문을 작성하지 않아도 컴파일 에러 발생 x
	 * - 대부분의 Unchecked Exception은 if문으로 해결 가능(사용자, 이용자의 실수 방지)
	 */
	public void test6() {
		// Unchecked Exception 확인하기 -> 컴파일 에러 없음
		 throw new RuntimeException();
		
		// Checked Exception 확인하기 -> 컴파일 에러(빨간줄) 발생
		// 에러내용 : Unhandled exception type Exception : 예외 처리 안했다는 오류 발생
