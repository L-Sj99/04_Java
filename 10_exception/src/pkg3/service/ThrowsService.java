...
public class ThrowsService {
	 ...
	 * 
	 * 1) 경고의 의미
	 *  -> 해당 메서드 기능을 활용하다보면 특정 예외가 발생할 수도 있으니 대비를 해라
	 * 
	 * 
	 * 2) 예외 처리를 한 곳에서 묶어 처리하기 위하여
	 * 
	 *  -> 실질적으로 예외를 처리하는 try - catch 구문이 사방에 흩어져 있을 경우 돌려 받는 최종 결과가 정상 결과인지, 예외 처리된 결과인지 구분하기 힘들어지는 경우가 발생
	 */
	...
	/**
	 * 전달 받은 num 값에 따라 IOException이 "발생할 가능성"이 있으므로 해당 메서드를 호출하는 곳에서는 
	...
	public void divNum() /* throws RuntimeException */{
		...
		// -> 근데 왜 빨간줄이 안뜨는 이유 : "Unchecked Exception" 이라서 빨간줄이 안뜬다
		
		// ArithmeticException 발생하면 호출한 곳으로 예외를 던지는 throws 사용
		
		// *** Unchecked Exception은 아무런 예외처리가 안되어 있는 경우 컴파일러가 자동으로 throws 구문을 생성해준다!! ***
	}
