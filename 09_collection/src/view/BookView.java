...
public class BookView {
	...
	public void mainMenu() {
		...
		do {
			...
			System.out.println("4. 저자로 조회(포함)");
			System.out.println("5. 가격으로 조회");
			System.out.println("6. 도서 추가");
			System.out.println("7. 도서 제거");
			System.out.println("8. 도서 가격 수정");
			...
			default : System.out.println("===== 잘 못 입력 =====");
			}
		} while(input != 0);
	} // mainMenu() method end
	...
	private void selectAll() {
		...
		for(int i = 0 ; i < bookList.size() ; i++) {
			// print() 관련 구문에서 참조 변수명을 작성하면 자동으로 toString()이 호출
	...
	private void selectIndex() {
		...
		// 전달한 index번재 요소가 존재하면 BookDTO 객체 주소 얻어오고 없으면 null(참조하는 객체가 없음) 반환
	...
	// 입력 받은 책 제목이 포함된 모든 책 출력하기, 없으면 "조회 결과가 없습니다" 출력
	...
	// 입력된 저자가 같은 책을 모두 조회하여 출력하기, 없으면 "조회 결과가 없습니다" 출력
	public void selectWriter() {
		System.out.println("\n----- 저자로 조회(포함) -----\n");
		String writer = sc.nextLine();
		List<BookDTO> searchList = service.selectTitle(writer);
	...
	public void selectPrice() {
		System.out.println("\n----- 가격으로 조회 -----\n");
		System.out.print("최소 금액을 작성해 주세요 : ");
		int min = sc.nextInt();
		System.out.println();
		System.out.print("최대 금액을 작성해 주세요 : ");
		int max = sc.nextInt();
 ...
 */
	...
	/**
	 * index 번호를 입력 받아 해당 index에 책이 존재하지 않으면 -> "해당 인덱스에 책이 존재하지 않습니다" 출력 후 메서드 종료
	 * 해당 index에 책이 존재하면 -> 수정할 가격을 입력 후 수정 ->"[책 제목] 가격이(이전 가격) -> (새 가격) 수정되었습니다." 형식으로 출력
	 */
	private void updateBook() {
		...
		// ex) index가 0인 경우 BookService의 bookList.get(0)에 저장된 주소와 아래 book에 저장된 주소가 "같음"
		BookDTO book = service.selectIndex(index);
		if(book == null) {
			System.out.println("해당 인덱스에 책이 존재하지 않습니다.");
			return;
