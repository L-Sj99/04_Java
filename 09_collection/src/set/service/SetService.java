...
public class SetService {
	...
	public void test1() {
		...
		System.out.println("삼정 제거 : " + set.remove("삼성"));
		System.out.println(set);
		System.out.println();
		...
		// 5. boolean isEnpty() : 비어있으면 true, 아니면 false
		...
		/** Set
		 * - index가 없음 -> index를 활용하는 메서드가 없다
		 * -> 원하는 요소 하나만을 꺼낼 수 없다
		 * 
		 * - 순서 유지가 안된다 -> 중복 허용 안한다
		 *  
		 */
	}
	/**
	...
	public void test2() {
		...
		/* 1. Iterator(반복자 이용) : 컬렉션에 저장된 요소를 하나씩 순차접근하는 기능을 제공하는 인터페이스 */
	 ...
	 * ** 1, 2 둘 다 수행 해야만함 ** 
	 * -> Java는 동일한 객체임을 비교할 때 동등(equals) / 동일(hashCode) 모두 true여야지만 동일한 객체로 인식한다
	 */
	public void test3() {
		...
		// 향상된 for문으로 하나씩 출력
		// [key point] : p4, p5가 중복된 데이터로 인식되어서 한 번만 출력 되는지 확인
		
		// 확인 1(equals, hashCode 오버라이딩 X) -> 유리가 중복되서 출력됨
		for(Person p : set) {
	...
	/* 로또 번호 생성기
	 * - 금액을 천원 단위로 입력 받아 천원당 1회씩 번호를 생성해서 List에 저장한 후 생성 종료 되는 시점에 한 번에 출랴ㅕㄱ
	 *  
	 * 단, 5회 출력 시 마다 구분 선 추가하기
	 *  
	 * <pre>
	 *  
	 * 금액 입력 : 3000
	...
	public void lottoNumberGenerator() {
		...
		// 입력 된 금액 천원당 1회 반복
		 for(int i = 0 ; i < price / 1000 ; i++) {
			...
			// 1회치에 해당되는 로또 번호 생성 후 lottoList에 추가
			Set<Integer> lotto = new TreeSet<Integer>();
		...
		for(int i = 0 ; i < lottoList.size() ; i++ ) {
			// print 구문에서 toString 안넣어도 알아서 추가해줌
			/* 6 / 11 / 16 / ... 출력 전에
			 * if(i % 5 == 0) {
				System.out.println("-----------------------");
			} */
