...
public class Person {
	 ...
	 * - 현재 객체와 매개변수로 전달 받은 다른 객체의 필드 값이 일치하는지 확인하는 용도로
	 ...
	 * 
	 * ex) sha-256 암호화 방식 -> 문자열을 전달하면 256바이트 같이 문자열로 변환
	 * 
	 */
	@Override
	public int hashCode() {
		...
		// 필드에 저장된 값을 이용해 hashCode를 생성하기 때문에 필드 값이 모두 같으면 동일한 hashCode가 반환된다
