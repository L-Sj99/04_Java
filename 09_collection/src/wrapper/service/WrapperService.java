...
// *(에스터 리스크) 미 작성 시 컴파일러가 자동으로 추가해주는 구문
import java.lang.*;
...
public class WrapperService {
	...
	public void test1() {
		...
		// -> 자바 9 버전부터 더 이상 사용되지 않으며, 제거대상으로 표시되었습니다
		...
		// List<int> list = new ArrayList<int>(); -> 제네릭에 기본 자료형 작성 시 오류 발생
		 ...
		 list.add(100); // (int)100 -> (Integer)100 : Auto Boxing이 자동으로 적용 됨
		 list.add(300);
	...
	public void test2() {
		...
		// JS에서 문자열을 숫자로 바꿀 때 Number("123")이용, parseInt
		System.out.println("[String -> 기본 자료형 변환]");
		// 다른 언어(특히 HTML, JS)에서 전달 된 값이 있을 경우 보통 String 형태로 전달 되어져서 오는 경우가 많음
		// 이를 Java에서 기본 자료형으로 변환하는 경우 사용/ 단, char(Character)는 존재하지 않음
		
		/** WrapperClass.parse자료형("문자열")
		 * parsing(파싱) : 성질(형태) 자체를 다른 것으로 변경 = String -> int
		 * casting(캐스팅, 형변환) : 같은 자료형 값의 범위 변경 
		 */
		...
		// 방법 1) 기본 자료형 + ""(String)
		int num5 = 500;
		String change1 = num5 + ""; // 권장하지는 않는 방법 : 
