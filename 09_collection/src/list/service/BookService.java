...
public class BookService {
	...
	// BookDTO 객체 참조 변수만을 저장하는 ArratList 객체 생성
	// -> <BookList> == 제네릭(타입 제한)
	...
	public BookService() {
		...
		bookList.add(new BookDTO("페스트", "알베르 카뮈", 13000));
		bookList.add(new BookDTO("너무 친한 친구들", "넬레 노이하우스", 185000));
		bookList.add(new BookDTO("자바의 정석", "남궁 성", 35000));
		bookList.add(new BookDTO("눈물의 여왕 대본집 세트", "박지은", 60000));
		bookList.add(new BookDTO("타우누스 시리즈 세트", "넬레 노이하우스", 300000));
	}
	...
	/**
	 * bookList요소의 제목 중 전달 받은 title이 포함된 책을 모두 반환
	 * @param title
	...
	public String removeBook(int index) {
		...
		/* method chaining : (연쇄) 메서드의 결과(반환 값)를 이용해 또 다른 메서드를 호출 */
