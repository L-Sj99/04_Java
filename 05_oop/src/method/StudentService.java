...
public class StudentService {
	...
	public void mainMenu() {
		...
		do {
			...
			case 1 : 
				std = registStudent();
				System.out.println(std.getName() + "학생 등록");
				break;
			case 2 : 
	...
	private Student registStudent() {
		...
		// student 변수에는 생성된 Student 객체를 참조하는 주소 값이 담겨져있다
		// -> reture Student = 주소 값을 반환( == 주소 값이 복사되어 호출한 곳으로 돌아감 )
		return student;
	}
	/* 학생 이름 수정 메서드 
	 * @param std : 전달받은 Student 객체 참조 주소 */
	private void updateName(Student std) {
		...
		// 이름을 홍길동 -> 김철수로 변경하시겠습니다?(y / n)
		System.out.printf("이름을 %s -> %s로 변경하시겠습니까?(y / n)", std.getName(), newName);
		
		// String.toUpperCase() -> 대문자로 변환
		// String.toLowerCase() -> 소문자로 변환
		char check = sc.next().toLowerCase().charAt(0);
		if(check == 'n') { // 'n' 가 입력된 경우
	...
	public void studyJava(Student std) {
		...
		System.out.println("현재 Javs 역량 점수 : " + std.getJava());
		...
		// 100점(MAX_VALUE) 초과 시 Student.MAX_VALUE 대입
		if(temp > Student.MAX_VALUE) {
			temp = Student.MAX_VALUE;
		}
		// 0점(MIN_VALUE) 미만 시 Student.MIN_VALUE 대입
	...
	private void studyHtml(Student std) {
		...
		System.out.print("증가 또는 감소한 HTML역량 점수 입력 : ");
		int score = sc.nextInt();
