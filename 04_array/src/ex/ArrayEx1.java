...
public class ArrayEx1 {
	 ...
	 * - 생성된 배열의 요소(묶여진 배열)는 인덱스 번호로 구분된다(index = 0부터 시작)
	 * - 생성된 배열을 활용하기 위해서는 배열을 참조하는 "참조형 변수"를 이용한다
	 * 
	 * [참조형 변수]
	 * - 값 x, 주소를 저장하는 변수
	 * - 변수 호출 시 값을 반환x -> 지정된 주소로 찾아가 해당 위치의 배열/객체를 참조
	...
	public void method1() {
		...
		// 배열 선언
		int[] arr; // Stack 메모리 영역에 int[] 자료형을 참조할 변수 arr을 선언
		
		// 배열 할당(생성)
		arr = new int[4]; // Heap 메모리 영역에 int가 4칸인 배열을 할당(생성) 후 생성된 배열의 주소를 arr에 대입
		...
		// for문을 이용해 arr 값 모두 출력
		for(int i = 0 ; i < 4 ; i++) {
			System.out.printf("arr[%d]의 값 : %d \n", i, arr[i]);
		}
		
		// js에서는 되는데 java에서는 안되는 것
		/* Java의 배열은 크기 조절 x
		 * 그냥 출력 하는것도 x
		 * 대입 하는 것도x
		 */
		// arr[4] = 999; // ArrayIndexOutOfBoundsException : 배열 인덱스 범위 초과 예외
		
	}
	/* 정수 4개를 입력 받아 배열 요소에 차례대로 대입 후 저장된 값 한번에 출력하기 */
	public void method2() {
		
		// 정수 4칸짜리 배열을 생성하여, 배열 주소를 arr 변수에 대입
		int[] arr = new int[4];
		...
		// debug : 프로그램 수행이 정상적으로 이루어지지 않은 경우 원인을 파악하기 위해서 추적 후 해결
		// 			또는 코드를 정상적으로 동작을 못하게 하는 이유 파악 후 추척 후 해결
	...
	public void method3() {
		...
		System.out.printf("당신이 시험볼 과목은 %s입니다.", subjects[ran]);
	}
	
	/* 5칸짜리 char배열을 만들어 각각의 배열 요소에 입력된 값을 대입
	...
	public void method4() {
		...
		for(int i = 0 ; i < arr.length ; i++) {
			...
			arr[i] = sc.next().charAt(0); // 입력된 문자열의 0번째 문자를 반환한다 -> char 입력 방법
		}
		// a p p l e -> e l p p a
