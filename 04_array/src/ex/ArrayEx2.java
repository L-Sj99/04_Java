...
public class ArrayEx2 {
	 ...
	 * - 서로 다른 변수가 같은 배열 / 객체를 참조하기 때문에 같은 값을 공유한다
	 */
	public void method1() {
		...
		// 복사한 배열 수정 했으나, 원본도 같이 수정됨 -> 얕은 복사는 주소만 복사해서 같은 배열을 참조하게 만들어 값을 공유
	...
	/* 깊은 복사
	 * - 원본 배열 / 객체가 가지고 있는 값을 똑같이 복사한 배열 / 객체를 만드는 것(복제의 개념)
	...
	public void method2() {
		...
		// 깊은 복사2 : System.arraycopy() 이용
		// System.arraycopy( 원본 배열명, 원복 복사 시작 인덱스, 복사 배열명, 복사 배열의 삽입 시작 인덱스, 복사 길이)
	 ...
	 * iterable 성격을 띄는 객체의 요소를 하나씩 순서대로 반환하는 for문(iterable : 반복 가능한)
	 * 
	 ...
	 * int[] arr = new int[3];
	 * 
	 * for( int num : arr ){ // num은 for문이 반복될 때마다 arr[0], arr[1], arr[2] 인덱스 값이 순차적으로 대입됨
	 * }
	 */
	public void method3() {
		int[] arr = {10, 20, 30, 40};
	...
	public void method5() {
		...
		// arr.length : arr가 참조하고 있는 1차원 배열 참조 변수의 묶음의 길이
