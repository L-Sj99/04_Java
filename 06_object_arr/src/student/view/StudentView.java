...
// 콘솔창에 보여지는 용도 == 클라이언트의 입력 / 출력을 담당하는 클래스
public class StudentView {
	...
	// 학생 추가 화면
	// 학번, 이름, 성별을 입력 받아 Student 객체로 만들어서 StudentService로 전달 -> 추가 성공 시 true / 실패시 false 반환 받아 결과 출력
	private void addStudent() {
		...
		// StudentService객체의 addStudent() 호출 후 결과 반환 받기
	...
	/* StudentService에 있는 student 객체 배열을 얻어와 모든 학생 정보를 출력 */
	private void allStudent() {
		...
		for(StudentDTO std : students) {
			...
			// -> NullPointerException 방지(참조 변수가 null을 저장하고 있는데 사용하려고 할 때)
			if(std == null) return;
	...
	/* 인덱스를 입력 받아 servicw에 전달하여 해당 인덱스의 학생 객체 하나(StudentDTO)를 반환 받아 출력 */
	...
	/* 이름을 입력 받아 service의 student 배열에서 같은 이름을 가진 학생 객체를 얻어와 출력 -> 이름 검색 */
	private void selectName() {
		System.out.println("\n----- 이름으로 조회 -----\n");
		System.out.print("검색할 학생 이름 입력 : ");
		String targetName = sc.nextLine();
		
		// 입력 받은 이름(targetName)을 서비스로 전달하여 일치하는 이름을 가진 학생 객체(주소 값) 반환 받기
		// -> null 또는 학생 객체(주소 값)
	 ...
	 * 단, 입력된 인덱스가 0 미만 students 배열 마지막 인덱스 초과한 경우 "인덱스 범위가 올바르지 않습니다." (1)
	 * 정상 범위 인덱스 이지만 학생이 존재하지 않는 경우
	 * "해당 인덱스에 학생 정보다 존재하지 않습니다" 출력 (2)
	 * 정상(3) */
	 ...
	 private void updateIndex(){
		 ...
		 if(check == 2) {
			 System.out.println("해당 인덱스에 학생 정보다 존재하지 않습니다.");
			 return;
		 }
	 ...
	 private void testMaxMin() {
		 ...
		 System.out.println("최고점 : " + max); // 70
		 System.out.println("최저점 : " + min); // 10
	 }
}
