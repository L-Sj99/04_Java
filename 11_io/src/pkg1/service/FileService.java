...
public class FileService {
	 ...
	 * Windows : \(백슬래시)
	 * Linux/Mac : /(슬래시)
	 * -> Java는 모든 운영체제가 호환되기 때문에 경로 표기 방법도 모두 호환된다
	 * 
	 * 최상위 경로(root) : /(슬래시) 또는 \(백슬래시)
	 * Windows : C드라이브, D드라이브...
	 * Linux/Mac : /, /Users
	 */
	...
	public void test2() {
		...
		// 경로 표기 방법 : \ 또는 /
		// \(백슬래시)는 문자열(String)에서 Escape 문자(\n, \t(탭키)...)를 위한 키워드이기 때문에
		// \ 문자열을 나타내고 싶으면 "\\" 두번 작성해야한다
		// "\\" -> \
		File file = new File("\\io_test\\202040731\\파일생성.txt");
		System.out.println("\\io_test\\202040731\\파일생성.txt");
	...
	public void test3() {
		...
		for(File f : files) {
			...
			// 마지막으로 수정한 날짜 -> 1970.01.01 09:00:00.00 기준으로 지난 시간만큼을 ms 단위로 반환
			...
			if(f.isFile()) { // 파일인 경우에만
				...
				// Byte를 -> KB(1024byte) 변경
				long size = fileSize / 1024;
				if(size < 1) size = 1; // 1KB 미만은 1KB 1로 표시
